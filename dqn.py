...
class ReplayBuffer():
    ...
    def sample(self, batch_size):
        ...
        states, actions, rewards, next_states, dones = (np.array(x) for x in zip(*sample))
...
class BaseAgent(Base):
    def act(self, state, epsilon=0.0):
        ...
        if pol < epsilon:
            ...
            return int(self.forward(state).argmax().item())
...
def compute_loss(model, target, states, actions, rewards, next_states, dones):
    ...
    action_Q = model(states).gather(dim=1, index=actions)
    next_state_Q = target(next_states).max(1)[0].view(-1, 1)
    target_Q = rewards + (1 - dones) * gamma * next_state_Q
    loss = criterion(action_Q, target_Q)
