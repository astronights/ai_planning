...
class GeneratePDDL_Stationary :
    ...
    def generateInitString(self) :
        ...
        '''  
        free_cells = self.grid_cell_list[:]
        initString = ''
        for car in self.state.cars:
            x, y = car.position.x, car.position.y
            initString = initString + f"(at pt{x}pt{y} car{car.id}) (blocked pt{x}pt{y}) "
            free_cells.remove(f"pt{x}pt{y}")
        for cell in free_cells:
            initString = initString + f"(not (blocked {cell})) "
        initString = initString + f"(at pt{self.state.agent.position.x}pt{self.state.agent.position.y} agent1) "
        for cell in self.grid_cell_list:
            ...
            new_x = cur_x if cur_x == 0 else cur_x - 1
            initString = initString + f"(up_next {cell} pt{new_x}pt{up_y}) (down_next {cell} pt{new_x}pt{down_y}) (forward_next {cell} pt{new_x}pt{cur_y})  "
        return initString
    ...
    def generateGoalString(self) :
        ...
        '''    
        goalString = f'(and (at pt{self.state.finish_position.x}pt{self.state.finish_position.y} agent1))'
        return goalString
...
def generateDomainPDDLFile(gen):
    ...
    gen.addAction(name="UP", parameters=(("pt1" , "gridcell"), ("pt2", "gridcell")), 
            precondition_string="(and (at ?pt1 agent1) (up_next ?pt1 ?pt2) (not (blocked ?pt2)))", 
            effect_string="(and (at ?pt2 agent1) (not (at ?pt1 agent1)))")

    gen.addAction(name="DOWN", parameters=(("pt1" , "gridcell"), ("pt2", "gridcell")),
            precondition_string="(and (at ?pt1 agent1) (down_next ?pt1 ?pt2) (not (blocked ?pt2)))",
            effect_string="(and (at ?pt2 agent1) (not (at ?pt1 agent1)))")

    gen.addAction(name="FORWARD", parameters=(("pt1" , "gridcell"), ("pt2", "gridcell")),
